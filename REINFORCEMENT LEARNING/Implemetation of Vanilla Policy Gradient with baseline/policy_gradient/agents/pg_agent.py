...
class PGAgent(BaseAgent):
    ...
    #from rl_trainer.py def train_agent(self)
    ...
    def calculate_q_vals(self, rews_list):
        ...
        if not self.reward_to_go:
            ...
            #q_values = np.concatenate([self.gamma*r for r in rews_list])

            q_values=np.concatenate(np.array([np.ones(len(l))*np.sum(np.array([(self.gamma**i)*c for i,c in enumerate(l)])) for l in rews_list]))
    ...
    def estimate_advantage(self, obs, q_values):
        ...
        if self.standardize_advantages:
            adv_n = (adv_n - np.mean(adv_n)) / (np.std(adv_n) + 1e-8) #mean zero with standard deviation 1
    ...
    def _discounted_return(self, rewards):
       ...
       # sum_of_discounted_rewards = TODO
        ...
        #list_of_discounted_returns =[np.ones(len(rewards))*np.sum(np.array([(self.gamma**i)*c for i,c in enumerate(rewards)]
        list_of_discounted_returns=list(np.ones(len(rewards))*np.sum(np.array([(0.1**i)*c for i,c in enumerate(rewards)])))
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for start_time_index in range(len(rewards)): 
            ...
            #print("t",start_time_index)
            indices = np.array(list(range(start_time_index,len(rewards),1)))
            #print("t'",indices)
            #print("t'-t",indices-start_time_index)
            discounts = np.power(self.gamma,indices-start_time_index)
            #print("discounts=",discounts)
            sum_discounted_rtg = np.matmul(discounts,rewards[start_time_index:len(rewards)])
            #print("rewards=",rewards[start_time_index:len(rewards)])
            #print(sum_discounted_rtg)
