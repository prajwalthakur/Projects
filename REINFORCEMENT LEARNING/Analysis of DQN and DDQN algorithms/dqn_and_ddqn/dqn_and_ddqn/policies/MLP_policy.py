...
from dqn_and_ddqn.infrastructure.tf_utils import build_mlp
...
class MLPPolicy(BasePolicy):
    ...
    def get_action(self, obs):
        ...
        x=self.sess.run(self.sample_ac, feed_dict={self.observations_pl : observation})
        #print("x=",x)
        return x
...
class MLPPolicyPG(MLPPolicy):
    ...
    def define_train_op(self):
        ...
        # HINT4: don't forget that we need to MINIMIZE this self.loss
            # but the equation above is something that should be maximize=
        ...
        if self.nn_baseline:
            ...
            #self.baseline_loss = 0.5*tf.reduce_sum(np.power((self.run_baseline_prediction(self.observations_pl)-self.targets_n),2))
    ...
    def run_baseline_prediction(self, obs):
        ...
        tf.squeeze(build_mlp(self.observations_pl, output_size=1, scope='nn_baseline', n_layers=self.n_layers, size=self.size))
        """   
        ...
        #else:
          #  observation = obs[None]
        x=self.sess.run(self.baseline_prediction, feed_dict={self.observations_pl : obs})
    ...
    # from pg_Agent.py  loss = self.actor.update(obs, acs, qvals=q_values, adv_n=advantage_values)
    def update(self, observations, acs_na, adv_n=None, acs_labels_na=None, qvals=None):
        ...
        if self.nn_baseline:
            print("self.baseline=",self.nn_baseline)
            print("inside_baseline=")
            targets_n = (qvals - np.mean(qvals))/(np.std(qvals)+1e-8)  #qvalues to state value target
            # TODO: update the nn baseline with the targets_n
            # HINT1: run an op that you built in define_train_op
            _, baseline_loss = \
                self.sess.run([self.baseline_update_op, self.baseline_loss], \
