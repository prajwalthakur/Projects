...
class SAC:
    ...
    """  def _value_function_loss_for(self, policy, q_function, q_function2, value_function):
        ...
        if q_function2 is None:
            ...
            return 0.5*tf.losses.mean_squared_error(q_values-self._alpha * log_pis,value_function(self._observations_ph))
        else:
            actions, log_pis = policy(self._observations_ph)
            q_values = q_function2((self._observations_ph, actions))
            return 0.5*tf.losses.mean_squared_error(value_function(self._observations_ph),tf.reduce_mean(q_values-self._alpha * log_pis))
    """
    ...
    """ def _q_function_loss_for(self, q_function, target_value_function):
        ...
        return 0.5*tf.losses.mean_squared_error(self._rewards_ph+(1-self._terminals_ph)*self._discount*target_value_function(self._next_observations_ph),q_function((self._observations_ph,self._actions_ph)))
    """  #return tf.keras.losses.MSE(q_function((self._observations_ph,self._actions_ph)),self._rewards_ph+self._discount*target_value_function(self._next_observations_ph))
