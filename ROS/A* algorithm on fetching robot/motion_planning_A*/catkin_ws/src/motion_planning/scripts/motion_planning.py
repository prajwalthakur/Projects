...
class MoveArm(object):
    ...
    def closet_point_toward_node(self,vector_ba,b_vec,a_vec,discrete_range):
        ...
        if(r1<0.2):
            ...
            #r4=r4+2
            #print("vector_ba",vector_ba)
            vector_points=numpy.array([vector_ba*discrete_range*i for i  in range(1,r4+2)])
            i=0
           ...
           # print("after",vector_points[0])
            ...
            while(i<row ): 
                if (self.is_state_valid(vector_points[i])==True):
                    ...
                    print("vector_points[i]",vector_points[i])
    ...
    def is_collision_free_path(self,a,b):
        vectorba=self.unit_vect(b,a)
        print("vectorba=",vectorba)
        closet_point_toward_b=self.closet_point_toward_node(vectorba,b,a,0.5)
        distance=numpy.linalg.norm((numpy.array(b)-closet_point_toward_b))
        if distance<0.5:
            status=True
        else:
            status=False
    ...
    def motion_plan(self, q_start, q_goal, q_min, q_max):
        ...
        while(p!=600 and flag!=1 ):
            ...
            if (self.is_state_valid(random_joint_point)):
                ...
                #print("d[points+str(p)].parent.data=",d["points"+str(index)].data)
                parent_to_child_vector= self.unit_vect(d["points"+str(p)].data,qv_list[index])
                #print("parent_to_child_vector",parent_to_child_vector)
                u=d["points"+str(p)].data
                closet_point=self.closet_point_toward_node(parent_to_child_vector,d["points"+str(p)].data,qv_list[index],random_range)
                d["points"+str(p)].data=closet_point
                node_t_goal_vector=self.unit_vect(goal.data,d["points"+str(p)].data) 
                
                print("d[points+str(p)].data ",u,"qv_list",qv_list[index],"parent_to_child_vector",parent_to_child_vector,"close ",closet_point," d[points+str(p)].data ",p," data",d["points"+str(p)].data)
                #print(" node_t_goal_vector", node_t_goal_vector)
                
                closet_point_to_goal=self.closet_point_toward_node(node_t_goal_vector,goal.data,d["points"+str(p)].data,random_range)
                distance_to_goal=numpy.linalg.norm((numpy.array(closet_point_to_goal)-numpy.array(goal.data)))
                #print(distance_to_goal)
                if(distance_to_goal<0.2):
                    print("meet")
                    goal.parent=d["points"+str(p)]
                    goal.distance_parent=numpy.linalg.norm((numpy.array(d["points"+str(p)].data),numpy.array(goal.data)))
                    flag=1
                p=p+1
            now = rospy.get_rostime().secs
        if(flag==1):
