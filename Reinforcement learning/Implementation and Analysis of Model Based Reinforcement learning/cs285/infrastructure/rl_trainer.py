...
class RL_Trainer(object):
    ...
    def run_training_loop(self, n_iter, collect_policy, eval_policy,
        ...
        for itr in range(n_iter):
            ...
            use_batchsize = self.params['batch_size'] #step size in one go batch
            if itr==0:
                use_batchsize = self.params['batch_size_initial'] #first time step size (put into replay buffer)
            ...
            # relabel the collected obs with actions from a provided expert policy; in MBRL expert policy is s(t+1) comes from PI(mpc) policy :see writeup
            if relabel_with_expert and itr>=start_relabel_with_expert:
                paths = self.do_relabel_with_expert(expert_policy, paths)

            # add collected data to replay buffer
            self.agent.add_to_replay_buffer(paths, self.params['add_sl_noise']) #add zero mean Gaussian noise to inputs and output labels as well (input:st,at;output:st+1-st)

            # train agent (using sampled data from replay buffer)
            all_losses = self.train_agent() #train dynamic model 
